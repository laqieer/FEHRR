...
def handle_face_clear_commands(text):
    # only one clear command follows a face command
    result = ''
    ss = text.split(' TEXT_CMD_LEFT TEXT_CMD_CLEAR ')
    for i, s in enumerate(ss):
        result += s
        if i != len(ss) - 1:
            result += ' TEXT_CMD_LEFT '
        if 'TEXT_CMD_FACE' in s:
            result += ' TEXT_CMD_CLEAR '
    return result
...
def convert_text(language, text):
    ...
    text = handle_face_clear_commands(text)
...
def write_texts(type, filename):
    text_type = type if type == TextType.SCENARIO else TextType.DATA
    with open(filename, 'w', encoding='utf-8') as f:
        f.write('// This file is generated by make_texts.py\n')
        f.write('// Text type: %s\n' % type)
        for i, key in enumerate(sorted(get_text_keys(type))):
            ...
            for language in Language:
                ...
                if type == TextType.STAGE:
                    if language == Language.LANGUAGE_ENGLISH:
                        text_prefix = '"Ch.%d: "' % (i + 1)
                    else:
                        text_prefix = '"第%d章 "' % (i + 1)
                if language in texts[text_type][key]:
                    f.write('    [%s] = %s,\n' % (language.name, text_prefix + texts[text_type][key][language]))
                else:
