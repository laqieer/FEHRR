...
def sort_units_and_skills():
    global units
    global skills
    units = sorted(unit_counts.keys(), key=lambda x: unit_counts[x], reverse=True)
    skills = sorted(skill_counts.keys(), key=lambda x: skill_counts[x], reverse=True)
